...
def getJavaOpts(includeAMQ, includeTransform, includeShare, solrHost, solrBaseUrl, sharding, communication):
    ...
    if communication in ('mtls', 'https'):
        commOpts = MTLS_OPTS
...
def getSolrcoreReplacements(sharding, communication, fingerprint):
    ...
    if communication == 'mtls':
        ...
        # SOLR_SSL_* env vars set in the compose file (no solrcore.properties edit needed).
        solrcoreReplacements['alfresco.secureComms=none'] = 'alfresco.secureComms=https'
    elif communication == 'none':
...
# ---------------------------------------------------------------------------
# Pristy https (PKCS12 mTLS) profile — uses keystores generated by
# keystore/generate-keystores.sh (solr.p12, trackers.p12, alfresco.p12,
# truststore.p12, password "changeit").  Mount the output dir read-only
# at /keystore inside each container.
# ---------------------------------------------------------------------------

KEYSTORE_PASS_HTTPS = 'changeit'

def addSolrHttpsConfig(solrEnvNode):
    """Configure the Solr service for Pristy PKCS12 mTLS (https profile)."""
    solrEnvNode['SOLR_SSL_ENABLED'] = 'true'
    solrEnvNode['ALFRESCO_SECURE_COMMS'] = 'https'
    solrEnvNode['SOLR_SSL_KEY_STORE'] = '/keystore/solr.p12'
    solrEnvNode['SOLR_SSL_KEY_STORE_PASSWORD'] = KEYSTORE_PASS_HTTPS
    solrEnvNode['SOLR_SSL_KEY_STORE_TYPE'] = 'PKCS12'
    solrEnvNode['SOLR_SSL_TRUST_STORE'] = '/keystore/truststore.p12'
    solrEnvNode['SOLR_SSL_TRUST_STORE_PASSWORD'] = KEYSTORE_PASS_HTTPS
    solrEnvNode['SOLR_SSL_TRUST_STORE_TYPE'] = 'PKCS12'
    solrEnvNode['SOLR_SSL_NEED_CLIENT_AUTH'] = 'true'

def addSolrHttpsVolumes(solrNode, keystoreDir):
    """Mount the PKCS12 keystore directory into the Solr container."""
    solrNode['volumes'] = ['{}:/keystore:ro'.format(keystoreDir)]

def addTrackersHttpsConfig(trackersEnvNode, keystoreDir):
    """Configure the trackers service for Pristy PKCS12 mTLS (https profile)."""
    # Solr-side tracker TLS (L2 SolrJ)
    trackersEnvNode['ALFRESCO_TRACKER_SOLR_SECURECOMMS'] = 'https'
    trackersEnvNode['ALFRESCO_TRACKER_SOLR_SSL_KEY_STORE'] = '/keystore/trackers.p12'
    trackersEnvNode['ALFRESCO_TRACKER_SOLR_SSL_KEY_STORE_PASSWORD'] = KEYSTORE_PASS_HTTPS
    trackersEnvNode['ALFRESCO_TRACKER_SOLR_SSL_KEY_STORE_TYPE'] = 'PKCS12'
    trackersEnvNode['ALFRESCO_TRACKER_SOLR_SSL_TRUST_STORE'] = '/keystore/truststore.p12'
    trackersEnvNode['ALFRESCO_TRACKER_SOLR_SSL_TRUST_STORE_PASSWORD'] = KEYSTORE_PASS_HTTPS
    trackersEnvNode['ALFRESCO_TRACKER_SOLR_SSL_TRUST_STORE_TYPE'] = 'PKCS12'
    # Repository-side tracker TLS (L3 repo calls)
    trackersEnvNode['ALFRESCO_TRACKER_REPOSITORY_SECURECOMMS'] = 'https'
    trackersEnvNode['ALFRESCO_TRACKER_REPOSITORY_SSL_KEY_STORE'] = '/keystore/trackers.p12'
    trackersEnvNode['ALFRESCO_TRACKER_REPOSITORY_SSL_KEY_STORE_PASSWORD'] = KEYSTORE_PASS_HTTPS
    trackersEnvNode['ALFRESCO_TRACKER_REPOSITORY_SSL_KEY_STORE_TYPE'] = 'PKCS12'
    trackersEnvNode['ALFRESCO_TRACKER_REPOSITORY_SSL_TRUST_STORE'] = '/keystore/truststore.p12'
    trackersEnvNode['ALFRESCO_TRACKER_REPOSITORY_SSL_TRUST_STORE_PASSWORD'] = KEYSTORE_PASS_HTTPS
    trackersEnvNode['ALFRESCO_TRACKER_REPOSITORY_SSL_TRUST_STORE_TYPE'] = 'PKCS12'
    # Tracker admin server TLS on :8085
    trackersEnvNode['TRACKER_SERVER_SSL_ENABLED'] = 'true'
    trackersEnvNode['TRACKER_SERVER_SSL_KEY_STORE'] = '/keystore/trackers.p12'
    trackersEnvNode['TRACKER_SERVER_SSL_KEY_STORE_PASSWORD'] = KEYSTORE_PASS_HTTPS
    trackersEnvNode['TRACKER_SERVER_SSL_KEY_STORE_TYPE'] = 'PKCS12'
    trackersEnvNode['TRACKER_SERVER_SSL_TRUST_STORE'] = '/keystore/truststore.p12'
    trackersEnvNode['TRACKER_SERVER_SSL_TRUST_STORE_PASSWORD'] = KEYSTORE_PASS_HTTPS
    trackersEnvNode['TRACKER_SERVER_SSL_TRUST_STORE_TYPE'] = 'PKCS12'
    trackersEnvNode['TRACKER_SERVER_SSL_CLIENT_AUTH'] = 'need'

def addTrackersHttpsVolumes(trackersNode, keystoreDir):
    """Mount the PKCS12 keystore directory into the trackers container."""
    trackersNode['volumes'] = ['{}:/keystore:ro'.format(keystoreDir)]

def addAlfrescoHttpsConfig(alfrescoEnvNode, alfrescoArgsNode, keystoreDir):
    """Configure the Alfresco repository service for Pristy PKCS12 mTLS (https profile)."""
    # JAVA_OPTS additions for https comms with Solr — appended later in getJavaOpts
    alfrescoArgsNode['SOLR_COMMS'] = 'https'
    alfrescoArgsNode['TRUSTSTORE_TYPE'] = 'PKCS12'
    alfrescoArgsNode['TRUSTSTORE_PASS'] = KEYSTORE_PASS_HTTPS
    alfrescoArgsNode['KEYSTORE_TYPE'] = 'PKCS12'
    alfrescoArgsNode['KEYSTORE_PASS'] = KEYSTORE_PASS_HTTPS

def addAlfrescoHttpsVolumes(alfrescoNode, keystoreDir):
    """Mount the PKCS12 keystore directory into the Alfresco container."""
    alfrescoNode['volumes'] = ['{}:/keystore:ro'.format(keystoreDir)]

def makeSearchNode(outputDirectory, nodeName, externalPort, params, communication, extraEnvironmentVars={}, solrcoreConfig=[], solrcoreReplacements={}, keystoreDir=None):
    # Create a dictionary for the template replacement.
    allParams = dict(params)
    allParams['SOLR_HOST'] = nodeName
    allParams['ALFRESCO_PORT'] = 8443 if communication in ('mtls', 'https') else 8080
    ...
    # Add legacy JCEKS mTLS configuration if required
    if communication == 'mtls':
    ...
    # Add Pristy PKCS12 https profile if required
    if communication == 'https':
        addSolrHttpsConfig(searchNodeYaml['environment'])
        addSolrHttpsVolumes(searchNodeYaml, keystoreDir or '/tmp/mtls-out')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-comm', '--communication', default='none',
                        help='Communication mode between SOLR and Alfresco Repository. '
                             '"https" uses Pristy PKCS12 mTLS (generate keystores first with '
                             'STORE_PASS=changeit keystore/generate-keystores.sh <dir>). '
                             '"mtls" uses the legacy JCEKS upstream mTLS.',
                        choices=['none', 'mtls', 'secret', 'https'])
    parser.add_argument('-kd', '--keystoreDir', default='/tmp/mtls-out',
                        help='Path to the directory containing PKCS12 keystores generated by '
                             'keystore/generate-keystores.sh (used only for --communication https). '
                             'This directory is mounted read-only at /keystore in the containers. '
                             'Default: /tmp/mtls-out')
    ...
    if args.communication == 'mtls':
        ...
        addAlfrescoHttpsConfig(
            dcYaml['services']['alfresco']['environment'],
            dcYaml['services']['alfresco']['build']['args'],
            args.keystoreDir)
        addAlfrescoHttpsVolumes(dcYaml['services']['alfresco'], args.keystoreDir)
    ...
    # For the https profile the Solr URL uses https and port 8983 (Solr's SSL port).
    solrUrl = 'https://{}:8983/solr'.format(solrHost) if args.communication == 'https' else 'http://{}:8983/solr'.format(solrHost)
    repoUrl = 'https://alfresco:8443/alfresco' if args.communication == 'https' else 'http://alfresco:8080/alfresco'
    trackersEnv = {
        'ALFRESCO_TRACKER_SOLR_URL': solrUrl,
        'ALFRESCO_TRACKER_SOLR_COLLECTIONS': 'alfresco,archive',
        'ALFRESCO_TRACKER_REPOSITORY_URL': repoUrl,
    }
    ...
    if args.communication == 'https':
        addTrackersHttpsConfig(trackersNode['environment'], args.keystoreDir)
        addTrackersHttpsVolumes(trackersNode, args.keystoreDir)
    dcYaml['services']['trackers'] = trackersNode
    ...
    # Copy the keystores (when using legacy JCEKS mTLS)
    if args.communication == 'mtls':
        copy_tree(scriptDir + '/keystores', args.output + '/keystores')
    # For the Pristy https (PKCS12) profile the operator must run
    # keystore/generate-keystores.sh <keystoreDir> before starting the stack.
    # The keystoreDir is mounted read-only at /keystore in each container.

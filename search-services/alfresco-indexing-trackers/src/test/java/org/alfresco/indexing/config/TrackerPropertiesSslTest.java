...
class TrackerPropertiesSslTest
{
    @Test
    void solrChannelIsIndependentFromRepository()
    {
        TrackerProperties props = new TrackerProperties();
        props.getSolr().setSecureComms("https");
        props.getRepository().setSecureComms("secret");
        assertEquals("https", props.getSolr().getSecureComms());
        assertEquals("secret", props.getRepository().getSecureComms());
    }

    @Test
    void sslConfigDefaultsToPkcs12()
    {
